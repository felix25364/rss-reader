...
Diese App ist für Android (insbesondere das Pixel 8) optimiert. Um sie zu bauen, führe folgende Schritte aus:

1. Stelle sicher, dass das Flutter SDK installiert ist und auf dem `stable`-Kanal liegt.
2. Navigiere in das Stammverzeichnis dieses Repositories.
3. Installiere alle Abhängigkeiten:
   ```bash
   flutter pub get
   ```
4. Generiere den Code für Riverpod und Drift (SQLite):
   ```bash
...
5. Baue das APK:
   ...
   ```
   *Alternativ kannst du die App direkt auf einem verbundenen Gerät über `flutter run` starten.*

**Hinweis:** Die `minSdkVersion` ist auf 21 konfiguriert und die `targetSdkVersion` auf 34, um sicherzustellen, dass das Google ML Kit korrekt funktioniert und die neusten Android 14 APIs unterstützt werden.
